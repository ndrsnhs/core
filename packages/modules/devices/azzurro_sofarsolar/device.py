...
from modules.devices.azzurro_sofarsolar import inverter
from modules.devices.azzurro_sofarsolar.config import Azzurro, AzzurroConfiguration, AzzurroCounterSetup, AzzurroInverterSetup
...
class Device(AbstractDevice):
    COMPONENT_TYPE_TO_CLASS = {
        "inverter": inverter.AzzurroInverter
    }

    def __init__(self, device_config: Union[Dict, Azzurro]) -> None:
        self.components = {}  # type: Dict[str, Union[counter.AzzurroCounter, inverter.AzzurroInverter]]
        try:
            self.device_config = dataclass_from_dict(Azzurro, device_config)
            ip_address = self.device_config.configuration.ip_address
            self.client = modbus.ModbusTcpClient_(ip_address, 502)
        except Exception:
            log.exception("Fehler im Modul "+self.device_config.name)

    def add_component(self, component_config: Union[Dict, AzzurroCounterSetup, AzzurroInverterSetup]) -> None:
        ...
        with MultiComponentUpdateContext(self.components):
            if len(self.components) == 1:
                for component in self.components:
                    if isinstance(self.components[component], inverter.AzzurroInverter):
                ...
                for component in self.components:
                    ...
                    inverter_power = 0
                for component in self.components:
                    if isinstance(self.components[component], counter.AzzurroCounter):
...
def read_legacy(component_type: str, ip_address: str, num: Optional[int] = None) -> None:
    dev = Device(Azzurro(configuration=AzzurroConfiguration(ip_address=ip_address)))
    ...
    if component_type in COMPONENT_TYPE_TO_MODULE and component_type == "counter":
        inverter_config = AzzurroInverterSetup()
        inverter_config.id = 1
        dev.add_component(inverter_config)

    log.debug('Azzurro IP-Adresse: ' + ip_address)
...
device_descriptor = DeviceDescriptor(configuration_factory=Azzurro)
